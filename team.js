/* ============================================================
   team.js — Anypedia Team Page Logic (Исправленная версия)
   ============================================================ */
...
function getSiteBasePath() {
  ...
  if (path.includes('/team/')) {
    return path.slice(0, path.lastIndexOf('/team/') + 1) || '/';
  }
...
function tournamentPageLink(tournamentId) {
  return `${getSiteBasePath()}${encodeURIComponent(tournamentId)}`;
}
...
function buildTournamentRows(team) {
  const rows = (team.tournaments || []).map(item => {
    ...
    return {
      ...
      dateLabel: item.date ? formatDateShort(item.date) : (found ? formatDateShort(found.start) : '—'),
      place: item.place || '—',
      title: item.title || '—',
...
/* ============================================================
   MAIN — FIND TEAM & RENDER (ИСПРАВЛЕНО)
   ============================================================ */
...
if (!team) {
  document.body.innerHTML = `
    <div style="height:100vh;display:flex;align-items:center;justify-content:center;background:#0f0f10;color:#eaeaea;text-align:center;font-family:'Inter',sans-serif">
      <div>
        <h1 style="font-size:22px;margin-bottom:10px">Команда не найдена</h1>
        <p style="color:#888;margin-bottom:20px">Проверьте ссылку или вернитесь на главную</p>
        <a href="/" style="padding:12px 20px;background:#1c1c1e;border:1px solid #333;border-radius:10px;color:#6bb6ff">← На главную</a>
      </div>
    </div>`;
...
if (new URLSearchParams(window.location.search).has('id') || !location.pathname.includes('/team/')) {
  history.replaceState(null, '', `${getSiteBasePath()}team/${encodeURIComponent(currentId)}`);
}
...
/* Фикс ссылки Anypedia в шапке */
const homeLink = document.getElementById('homeLink');
if (homeLink) homeLink.href = basePath || '/';

/* Инфобокс */
document.getElementById('teamInfobox').innerHTML = `
  <img class="team-logo" src="${basePath}${escapeHtml(team.logo || 'dota2.png')}" alt="${escapeHtml(team.name)}" onerror="this.style.display='none'">
  <h1>${escapeHtml(team.name)}</h1>
  <div class="info-row"><span class="info-label">Регион</span><span class="info-val">${escapeHtml(team.region || '—')}</span></div>
  <div class="info-row"><span class="info-label">Призовые</span><span class="info-val" style="color:var(--accent);font-weight:700">${escapeHtml(team.prize || '—')}</span></div>
  <a class="btn btn-tg ${team.telegramLink ? '' : 'is-disabled'}" href="${escapeHtml(team.telegramLink || '#')}" target="_blank" rel="noopener">✈️ Telegram</a>
  <a class="btn btn-captain ${team.captainLink ? '' : 'is-disabled'}" href="${escapeHtml(team.captainLink || '#')}" target="_blank" rel="noopener">👤 Связаться с капитаном</a>
`;
